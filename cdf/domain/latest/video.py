...
class Camera(TypedDict):
    x: NotRequired[Optional[float]]  # x location of the camera in relation to the pitch center (m)
    y: NotRequired[Optional[float]]  # y location of the camera in relation to the pitch center (m)
    z: NotRequired[Optional[float]]  # z location of the camera in relation to the pitch center (m)


class Recording(TypedDict):
    fps: int  # Frames per second (i.e. frame rate) from vendor
    resolution: str  # Resolution of the video in pixels (e.g., 3840x2160, 1920x1080, ...)
    start_time: str  # The start time in UTC of the recording
    type: str  # Information how the video was recorded (e.g. fixed camera, camcorder,...)
    operation_type: str  # Information whether the camera operation was manual or automated
