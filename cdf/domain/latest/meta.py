...
#   filename:  meta.json
#   timestamp: 2025-10-16T08:06:38+00:00
...
class Competition(TypedDict):
    ...
    format: NotRequired[str]  # Format of the competition (e.g., 'league_18', 'league_20')
    age_restriction: NotRequired[Optional[str]]  # Age restriction for the competition (e.g., 'U18', 'U20')
...
class Period(TypedDict):
    ...
    play_direction: Literal['left_right', 'right_left']  # The direction of play for the home team
    ...
    left_team_id: NotRequired[str]  # Unique team identifier of the team playing on the left side of the pitch in this period. The left_team_id and right_team_id should be the actual, nonstandardized sides.
    right_team_id: NotRequired[str]  # Unique team identifier of the team playing on the left side of the pitch in this period. The left_team_id and right_team_id should be the actual, nonstandardized sides.
...
class Misc(TypedDict):
    ...
    is_open_roof: NotRequired[bool]  # Indicates if the roof is open (true) or closed (false)
...
class Stadium(TypedDict):
    ...
    pitch_length: NotRequired[Optional[float]]  # Length of the pitch in metres, null if not available
    pitch_width: NotRequired[Optional[float]]  # Width of the pitch in metres, null if not available
    name: NotRequired[str]  # Name of the stadium
    turf: NotRequired[str]  # Information on the turf of the pitch (e.g., 'grass', 'natural_reinforced')
...
class Event(TypedDict):
    collection_timing: str  # Indicates if the event data was collected live or post match


class Tracking(TypedDict):
    version: str  # Version number for the tracking data collection in use (e.g. '0.1.0')
    name: str  # Vendor name of the tracking data
    fps: int  # Frames per second (i.e., frame rate) of tracking
    collection_timing: str  # Indicates if the tracking data was collected live or post match
...
class Limb(TypedDict):
    ...
    collection_timing: str  # Indicates if the limb data was collected live or post match
...
class Ball(TypedDict):
    ...
    collection_timing: str  # Indicates if the ball data was collected live or post match
...
class Meta(TypedDict):
    ...
    event: NotRequired[Optional[Event]]  # Event data meta information, null if not relevant
    tracking: Optional[Tracking]  # Tracking data meta information, null if not relevant
    limb: NotRequired[Optional[Limb]]  # Limb tracking data meta information, null if not relevant
    ball: NotRequired[Optional[Ball]]  # Ball tracking data meta information, null if not relevant. Only relevant when providing an independent ball file.
